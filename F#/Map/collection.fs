...
module Map =
    ...
    let private height = function
        | Node(_, _, _, _, hl, hr) -> max hl hr + 1
        | Empty -> 0

    let private node l r x y = Node(l, r, x, y, height l, height r)
    ...
    type Map<'key, 'value  when 'key: comparison and 'value: equality>private(t:Tree<'key, 'value>) = 
        ...
        member this.Remove(x: 'key) =
            ...
            let rec remove tree x =
                ...
                | Empty -> Empty
                | Node(a, b, m, n, l, r) -> 
                    if m = x then let c = delNode tree
                                  match c with
                                  | Empty -> c
                                  | Node(f, _, _, _, hl, hr) -> 
                                      if hl > hr + 1 then balance c [false; findWay f]
                                      else c
                    else 
                        if x > m then 
                            let mutable c = remove b x
                            let mutable h = r
                            match c with
                            | Empty -> h <- 0
                            | Node(_, _, _, _, hl, hr) -> h <- max hl hr + 1
        ...
        override this.ToString() =
            let rec toString = function
                | Empty -> "Empty"
                | Node(a, b, m, n, _, _) -> 
                    "Node(" + toString a + ", " + toString b + ", " + m.ToString() + ", " + n.ToString() + ")"
            toString t
